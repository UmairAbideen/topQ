        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="d-flex justify-content-center">
                    <div class="p-2">
                        <a href="{{ route('director.ia.schedule.view') }}" class="btn btn-outline-secondary btn-sm"
                                ...
                                <tbody>
                                    @foreach ($reports as $report)
                                        <tr>
                                            ...
                                            <td class="align-middle text-center text-sm">
                                                @if (!is_null($report->prepared_by) && is_null($report->approved_by))
                                                    <a href="{{ route('director.ia.report.approve', $report->id) }}"
                                                        class="btn bg-gradient-info btn-sm mb-0 ms-1 me-1 " role="button"
                                                        aria-pressed="true">Approve</a>
                                                @elseif (is_null($report->prepared_by))
                                                    Pending
                                                @else
                                                    {{ $report->approved_by }}
                                                @endif
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    @if ($report->approved_by)
                                                        <div>
                                                            <a href="{{ route('director.ia.report.download', $report->id) }}"
                                                                class="btn bg-gradient-success btn-sm mb-0 ms-1 me-1 "
                                                                role="button" aria-pressed="true">Download</a>
                                                        </div>
                                                    @endif
                                                    <div>
                                                        <a href="{{ route('director.ia.report.print', $report->id) }}"
                                                            target="_blank"
