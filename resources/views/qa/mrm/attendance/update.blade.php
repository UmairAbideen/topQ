                ...
                <div class="card my-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 my-0 z-index-2">
                        ...
                        <div class="d-flex justify-content-end pe-0 pt-4">
                            <a href="{{ route('qa.mrm.attendance.view', $attendance->agenda->id) }}" class="btn bg-gradient-info" role="button"
                        ...
                        <form class='px-3' action="{{ route('qa.mrm.attendance.update', $attendance->id) }}"
                            method="post">
                            @csrf
                                        ...
                                        <div class="input-group input-group-static mb-4">
                                            ...
                                            <select class="form-control" name="name{{ $i }}">
                                                ...
                                                @foreach ($names as $name)
                                                    <option value="{{ $name }}"
                                                        {{ $attendance->{'name' . $i} == $name ? 'selected' : '' }}>
                                                        {{ $name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-control" name="department{{ $i }}">
                                                ...
                                                @foreach ($departments as $department)
                                                    <option value="{{ $department }}"
                                                        {{ $attendance->{'department' . $i} == $department ? 'selected' : '' }}>
                                                        {{ $department }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6 px-3">
                                        <div class="input-group input-group-static mb-4">
                                            ...
                                            <select class="form-control" name="absence{{ $i }}">
                                                ...
                                                <option value="yes"
                                                    {{ $attendance->{'absence' . $i} == 'yes' ? 'selected' : '' }}>Yes
                                                </option>
                                                <option value="no"
                                                    {{ $attendance->{'absence' . $i} == 'no' ? 'selected' : '' }}>No
                                                </option>
                                            </select>
                                            @error('absence' . $i)
                                                <div class="text-danger small">{{ $message }}</div>
